...
class DiscreteToContinuousActionWrapper(gym.ActionWrapper):
    """Expose Discrete actions as a 1D continuous Box for SAC."""

    def __init__(self, env):
        super().__init__(env)
        assert isinstance(env.action_space, gym.spaces.Discrete), "wrapper expects a Discrete action space"
    ...
    def action(self, action):
        ...
        scaled = (value + 1.0) * 0.5 * self._num_actions
        discrete_action = int(np.clip(np.floor(scaled), 0, self._num_actions - 1))
...
def make_env(env_id, seed, idx, capture_video, run_name, vlm_goal, vlm_device, vlm_skip_frames):
    def thunk():
        ...
        env = VLMRewardWrapper(env, text_goal=vlm_goal, device=vlm_device, skip_frames=vlm_skip_frames)
