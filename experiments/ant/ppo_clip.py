# PPO with VLM Rewards on Ant-v4
# Adapted from CleanRL continuous-action PPO for Gymnasium MuJoCo Ant.
# Ant docs: https://gymnasium.farama.org/environments/mujoco/ant/
...
def make_env(env_id, idx, capture_video, run_name, vlm_goal, vlm_device):
    def thunk():
        # Use rgb_array so the VLM wrapper can access rendered frames.
        ...
        # Replace or shape the reward with the VLM reward.
        ...
        # Record episode stats after reward shaping.
        env = gym.wrappers.RecordEpisodeStatistics(env)

        # Continuous-control friendly wrappers.
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter("runs/{0}".format(run_name))
    writer.add_text(
        "hyperparameters",
        "|param|value|\n|-|-|\n{0}".format(
            "\n".join(
