...
def make_env(env_id, idx, capture_video, run_name, vlm_goal, vlm_device): # Removed VLM args
    def thunk():
        # Keep rgb_array so we can call env.render() later!
        env = gym.make(env_id, render_mode="rgb_array") 
        if capture_video and idx == 0:
            env = gym.wrappers.RecordVideo(env, f"videos/{run_name}", episode_trigger=lambda x: x % 50 == 0)
        env = gym.wrappers.RecordEpisodeStatistics(env) 

        # Wrap with VLM Reward Wrapper (Pass VLM args here)
