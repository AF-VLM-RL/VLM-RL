# RPO with VLM Rewards for CartPole
# RPO (Robust Policy Optimization) + CLIP-based VLM reward shaping.
import os
import random
import sys
import time
...
class Args:
    ...
    vlm_device: str = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    rpo_alpha: float = 0.5
    """the alpha parameter for RPO (noise added to logits during policy update)"""
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None):
        ...
        if action is None:
            ...
            z = torch.empty_like(logits).uniform_(-self.rpo_alpha, self.rpo_alpha).to(logits.device)
            perturbed_logits = logits + z
            probs = Categorical(logits=perturbed_logits)
            return action, probs.log_prob(action), probs.entropy(), self.critic(x)
...
if __name__ == "__main__":
    ...
    envs = gym.vector.SyncVectorEnv(
        [
            make_env(args.env_id, i, args.capture_video, run_name, args.vlm_goal, args.vlm_device)
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if "final_info" in infos:
                ...
                for i, done_flag in enumerate(infos.get("_episode", [])):
                    if done_flag:
