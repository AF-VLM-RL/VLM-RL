# PPO with VLM frame-sequence rewards for continuous action environments.
# FrameBufferWrapper collects composites each step; after each rollout, composites are
# scored in chunked batched VLM calls (not during env.step).
# docs and experiment results: https://docs.cleanrl.dev/rl-algorithms/ppo/#ppo_continuous_actionpy
import os
import random
import sys
import time
...
class Args:
    ...
    """Device for VLM (cuda/cpu). None = same as PPO device. Use 'cpu' when GPU is crowded."""
...
def make_env(env_id, idx, capture_video, run_name, vlm_max_frames, vlm_layout, vlm_grid_size, vlm_frame_stride):
    def thunk():
        ...
        # New lightweight buffer wrapper
...
if __name__ == "__main__":
    ...
    if args.track:
        ...
        # Force offline on SLURM (compute nodes often can't reach wandb servers)
    ...
    # VLM rewards: collect composites each step during rollout, then batch-score once
    # per iteration in chunks (see score_rollout_composite_images).
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # Collect active buffers for deferred VLM scoring
        ...
        # Flatten in **step-major, env-minor** order (same layout as `rewards` / C-order reshape):
        #   flat_images[s * num_envs + e] == composite for (step s, env e)
        # The inner loop over `e` only **lists** separate images; the VLM still gets one image
        # per batch row (not a single image stitched from all envs). Chunking slices this list.
        flat_images = [
